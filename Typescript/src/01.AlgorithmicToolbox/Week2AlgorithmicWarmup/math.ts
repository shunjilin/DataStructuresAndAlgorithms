...
const fibonacciModulo = (n: number, modulo: number) => {
  let pair: [number, number] = [0, 1]
  for (let i = 0; i < n; ++i) {
    const [a, b] = pair
    pair = [b, (a + b) % modulo]
...
export const fibonacciLastDigit = (n: number) => {
...
export const pisanoPeriod = (modulo: number) => {
  if (modulo === 1) {
    return 1
  }
  let pair: [number, number] = [0, 1]
...
export const fibonacciPartialSumLastDigit = (n: number, m: number) => {
  if (m < n) {
...
export const fibonacciSumOfSquaresLastDigit = (n: number) => {
